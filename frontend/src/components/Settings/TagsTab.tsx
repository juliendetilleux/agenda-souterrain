...
import type { CalendarConfig, Tag } from '../../types'
...
export default function TagsTab({ calendar }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      calendarApi.createTag(calendar.id, { name: newName, color: newColor }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tags', calendar.id] })
      setNewName('')
      toast.success(t('tags.toast.created'))
    },
    onError: () => toast.error(t('tags.toast.createError')),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name?: string; color?: string } }) =>
      calendarApi.updateTag(calendar.id, id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tags', calendar.id] })
      setEditingId(null)
      toast.success(t('tags.toast.updated'))
    },
    onError: () => toast.error(t('tags.toast.updateError')),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: string) => calendarApi.deleteTag(calendar.id, id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tags', calendar.id] })
      toast.success(t('tags.toast.deleted'))
    },
    onError: () => toast.error(t('tags.toast.deleteError')),
  })
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        {/* List */}
        {tags.length === 0 ? (
          <p className="text-sm text-stone-400 py-4 text-center">{t('tags.empty')}</p>
        ) : (
              ...
              >
                {editingId === tag.id ? (
                  ...
                  <>
                    ...
                    />
                    <span className="text-sm text-stone-700 flex-1 font-medium">{tag.name}</span>
                    <button
                    ...
                    <button
                      onClick={() =>
                        confirm(
                          t('tags.deleteTitle'),
                          t('tags.deleteMsg', { name: tag.name }),
